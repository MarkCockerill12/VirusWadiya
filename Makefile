...
# # Compile login.o (normal compilation)
# login.o: login.cpp
# 	$(C) $(CPPFLAGS) $(OPENSSL_FLAGS) -c $< -o $@
...
# # Decrypt login-subverted.cpp to login-subverted-tmp.cpp (with valid extension)
# login-subverted-tmp.cpp: login-subverted.cpp
# 	openssl enc -d -aes-256-cbc -in $< -out $@ -pass pass:SECRET_PASSWORD

# # Compile the decrypted login-subverted-tmp.cpp into login-subverted.o
# login-subverted.o: login-subverted-tmp.cpp
# 	$(C) $(CPPFLAGS) $(OPENSSL_FLAGS) -c $< -o $@
# 	rm -f $<  # Clean up the temporary decrypted .cpp file after compilation

# # Link login-subverted.o and authlib.o into the final executable login-subverted
